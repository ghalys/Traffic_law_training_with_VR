...
public class VehicleRobot : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.transform.root.GetComponent<VehicleRobot>())
        {

            collision.transform.root.GetComponent<VehicleRobot>().slip2 = Mathf.Clamp(collision.relativeVelocity.magnitude, 0.0f, 10.0f);
    ...
    void OnCollisionStay(Collision collision)
    {
       ...
       if (collision.transform.root.GetComponent<VehicleRobot>())
            collision.transform.root.GetComponent<VehicleRobot>().slip2 = 5.0f;
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            if (shift && (currentGear > 1 && speed > 50.0f) && shifmotor && Mathf.Abs(steer) < 0.2f)
            {
                ...
                var emission1 = carParticles.shiftParticle1.emission;
                emission1.rateOverTime = Mathf.Lerp(emission1.rateOverTime.constant, powerShift > 0 ? 50 : 0, Time.deltaTime * 10.0f);
                var emission2 = carParticles.shiftParticle2.emission;
                emission2.rateOverTime = Mathf.Lerp(emission2.rateOverTime.constant, powerShift > 0 ? 50 : 0, Time.deltaTime * 10.0f);
            ...
            else
            {
                ...
                var emission1 = carParticles.shiftParticle1.emission;
                emission1.rateOverTime = Mathf.Lerp(emission1.rateOverTime.constant, 0, Time.deltaTime * 10.0f);
                var emission2 = carParticles.shiftParticle2.emission;
                emission2.rateOverTime = Mathf.Lerp(emission2.rateOverTime.constant, 0, Time.deltaTime * 10.0f);
            }
                ...
                if (carParticles.brakeParticlePerfab)
                {
                    ...
                    for (int i = 0; i < carSetting.hitGround.Length; i++)
                    {
                        ...
                        if (hit.collider.CompareTag(carSetting.hitGround[i].tag))
                        {
                            ...
                            ParticleSystem.MainModule mainModule = Particle[currentWheel].GetComponent<ParticleSystem>().main;
                            mainModule.startColor  = carSetting.hitGround[i].brakeColor;
                    ...
                    {
                        ParticleSystem.EmissionModule emission = pc.emission;
